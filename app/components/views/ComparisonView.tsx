...
const ComparisonView = ({
  ...
  const handleChartClick = (event: any, chartRef: any, groupLabels: string[]) => {
  ...
  const comparisonList = useMemo(() => {
    // Filter
    const filtered = baseComparisonList.filter(item => 
  ...
  const provinceAggregation = useMemo(() => {
      const provinceMap = new Map<string, {
          ...
          successA_H: number; successA_M: number;
          successB_H: number; successB_M: number;
          callA_Q: number; callB_Q: number;
          group: string;
      }>();
      ...
      comparisonList.forEach(item => {
          ...
          const existing = provinceMap.get(mapKey) || {
              volA: 0, volB: 0,
              successA_H: 0, successA_M: 0,
              successB_H: 0, successB_M: 0,
              callA_Q: 0, callB_Q: 0,
              group: mapKey
          };
      ...
      return Array.from(provinceMap.values()).map(p => ({
          ...
          volA: p.volA,
          volB: p.volB,
          successRateA: p.successA_H > 0 ? (p.successA_M / p.successA_H) * 100 : 0,
          successRateB: p.successB_H > 0 ? (p.successB_M / p.successB_H) * 100 : 0,
  ...
  const tableSummary = useMemo(() => {
      ...
      let volA = 0, volB = 0;
      let successA_H = 0, successA_M = 0;
      let successB_H = 0, successB_M = 0;
      let callA_Q = 0, callB_Q = 0;
      ...
      return {
          volA, volB, diffVol: volB - volA,
          successRateA, successRateB, diffSuccess: successRateB - successRateA,
          callRateA, callRateB, diffCall: callRateB - callRateA
      };
  }, [comparisonList]);
        ...
        <div className="flex flex-col gap-8 mb-8 slide-up delay-200">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 flex flex-col relative overflow-hidden group z-0">
                    ...
                    <h3 className="text-lg font-bold text-slate-700 mb-1 flex items-center gap-2">
                        📦 ปริมาณงานแยกตาม{isSingleGroup ? `ที่ทำการ (${singleGroupName})` : 'จังหวัด (Volume by Region)'}
                    </h3>
                    <p className="text-sm text-slate-500 mb-4">แสดงปริมาณงานทั้งหมดที่เข้าสู่ระบบ เปรียบเทียบสองช่วงเวลาเพื่อดูแนวโน้มภาระงาน</p>
                        ...
                        <Bar
                            ...
                            options={{
                                ...
                                plugins: {
                                    ...
                                    tooltip: {
                                        callbacks: {
                                            label: (context) => `${context.dataset.label}: ${context.raw?.toLocaleString()} ชิ้น`
                                        }
                                    },
                                    datalabels: {
                                        color: '#334155',
                                ...
                                onClick: (event) => handleChartClick(event, chart1Ref, provinceAggregation.map(p => p.group)),
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 flex flex-col relative overflow-hidden group z-0">
                    ...
                    <h3 className="text-lg font-bold text-slate-700 mb-1 flex items-center gap-2">
                        ⚡ เปรียบเทียบประสิทธิภาพนำจ่ายตาม{isSingleGroup ? `ที่ทำการ (${singleGroupName})` : 'จังหวัด'}
                    </h3>
                    <p className="text-sm text-slate-500 mb-4">สัดส่วนของงานที่พนักงานนำจ่ายสำเร็จ</p>
                    <div className="h-[350px] w-full mt-auto">
                        <ReactChart
                            ref={chart2Ref}
                            type="bar"
                            ...
                            options={{
                                ...
                                onClick: (event) => handleChartClick(event, chart2Ref, provinceAggregation.map(p => p.group)),
                                onHover: (event, elements) => {
                                    if (!isSingleGroup) {
                                        (event.native?.target as HTMLElement).style.cursor = elements[0] ? 'pointer' : 'default';
                                    }
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 flex flex-col relative overflow-hidden group z-0">
                    ...
                    <h3 className="text-lg font-bold text-slate-700 mb-1 flex items-center gap-2">
                        📞 เปรียบเทียบประสิทธิภาพการโทรตาม{isSingleGroup ? `ที่ทำการ (${singleGroupName})` : 'จังหวัด'}
                    </h3>
                    <p className="text-sm text-slate-500 mb-4">สัดส่วนของการโทรติดต่อลูกค้า</p>
                    <div className="h-[350px] w-full mt-auto">
                        <ReactChart
                            ref={chart3Ref}
                            type="bar"
                            ...
                            options={{
                                ...
                                onClick: (event) => handleChartClick(event, chart3Ref, provinceAggregation.map(p => p.group)),
                                onHover: (event, elements) => {
                                    if (!isSingleGroup) {
                                        (event.native?.target as HTMLElement).style.cursor = elements[0] ? 'pointer' : 'default';
                                    }
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 flex flex-col relative overflow-hidden group z-0">
                    ...
                    <h3 className="text-lg font-bold text-slate-700 mb-1 flex items-center gap-2">
                        📈 ที่ทำการที่ % เปลี่ยนแปลงมากที่สุด (Top Movers)
                    </h3>
                    <p className="text-sm text-slate-500 mb-4">5 ที่ทำการที่ก้าวหน้าที่สุด และ 5 ที่ทำการที่ตกลงมากที่สุด</p>
                    <div className="h-[350px] w-full mt-auto">
                        {(() => {
                            const sortedByDiff = comparisonList.slice().sort((a, b) => b.diffSuccess - a.diffSuccess);
                            const top5 = sortedByDiff.slice(0, 5).filter(x => x.diffSuccess > 0);
                            const bottom5 = sortedByDiff.slice(-5).reverse().filter(x => x.diffSuccess < 0);
                                ...
                                <Bar
                                        ...
                                        datasets: [
                                            {
                                                ...
                                                backgroundColor: chartData.map(d => 
                                                    d.diffSuccess > 0 ? 'rgba(16, 185, 129, 0.6)' : 'rgba(244, 63, 94, 0.6)'
                                                ),
                                                borderColor: chartData.map(d => 
                                                    d.diffSuccess > 0 ? 'rgb(16, 185, 129)' : 'rgb(244, 63, 94)'
                                                ),
                                                borderWidth: 1,
                                                borderRadius: 4,
                                            }
                                    ...
                                    options={{
                                        ...
                                        plugins: {
                                            ...
                                            tooltip: {
                                                callbacks: {
                                                    label: (context) => `${Number(context.raw) > 0 ? '+' : ''}${Number(context.raw).toFixed(2)}%`
                                                }
                                            },
                                            datalabels: {
                                                color: (context) => Number(context.dataset.data[context.dataIndex]) > 0 ? '#047857' : '#be123c',
                                                anchor: (context) => Number(context.dataset.data[context.dataIndex]) > 0 ? 'end' : 'start',
                                                align: (context) => Number(context.dataset.data[context.dataIndex]) > 0 ? 'end' : 'start',
                                                font: { size: 10, weight: 'bold' },
         ...
         <div className="w-full">
             <table className="min-w-full text-sm relative">
                 <thead className="sticky top-[80px] z-40 bg-white shadow-md ring-1 ring-slate-200">
                     ...
                     <tr className="bg-slate-50 border-b border-slate-200">
                         {/* Volume Group */}
                         <th onClick={() => handleSort("volA")} className="py-3 px-2 text-center font-semibold text-slate-600 bg-blue-50/50 cursor-pointer hover:bg-blue-100 transition-colors">
                             <div className="flex items-center justify-center gap-1">
                                 {dateA ? dateA.toLocaleDateString('th-TH', { day: 'numeric', month: 'short' }) : '...'}
                                 <SortIcon col="volA" />
                             </div>
                         </th>
                         <th onClick={() => handleSort("volB")} className="py-3 px-2 text-center font-semibold text-slate-700 bg-purple-50/50 cursor-pointer hover:bg-purple-100 transition-colors">
                             <div className="flex items-center justify-center gap-1">
                                 {dateB ? dateB.toLocaleDateString('th-TH', { day: 'numeric', month: 'short' }) : '...'}
                                 <SortIcon col="volB" />
                             </div>
                         </th>
                         <th onClick={() => handleSort("diffVol")} className="py-3 px-4 text-center font-bold text-slate-700 cursor-pointer hover:bg-slate-200 bg-slate-100/50 border-r border-slate-200 transition-colors">
                             <div className="flex items-center justify-center gap-1">
                         ...
                         {/* Success Rate Group */}
                         <th onClick={() => handleSort("successRateA")} className="py-3 px-2 text-center font-semibold text-emerald-700 bg-emerald-50/50 cursor-pointer hover:bg-emerald-100 transition-colors">
                             <div className="flex items-center justify-center gap-1">
                                 {dateA ? dateA.toLocaleDateString('th-TH', { day: 'numeric', month: 'short' }) : '...'}
                                 <SortIcon col="successRateA" />
                             </div>
                         </th>
                         <th onClick={() => handleSort("successRateB")} className="py-3 px-2 text-center font-semibold text-emerald-700 bg-purple-50/50 cursor-pointer hover:bg-purple-100 transition-colors">
                             <div className="flex items-center justify-center gap-1">
                                 {dateB ? dateB.toLocaleDateString('th-TH', { day: 'numeric', month: 'short' }) : '...'}
                                 <SortIcon col="successRateB" />
                             </div>
                         </th>
                         <th onClick={() => handleSort("diffSuccess")} className="py-3 px-4 text-center font-bold text-emerald-700 cursor-pointer hover:bg-emerald-200 bg-emerald-100/50 border-r border-slate-200 transition-colors">
                             <div className="flex items-center justify-center gap-1">
                         ...
                         {/* Call Rate Group */}
                         <th onClick={() => handleSort("callRateA")} className="py-3 px-2 text-center font-semibold text-indigo-700 bg-indigo-50/50 cursor-pointer hover:bg-indigo-100 transition-colors">
                             <div className="flex items-center justify-center gap-1">
                                 {dateA ? dateA.toLocaleDateString('th-TH', { day: 'numeric', month: 'short' }) : '...'}
                                 <SortIcon col="callRateA" />
                             </div>
                         </th>
                         <th onClick={() => handleSort("callRateB")} className="py-3 px-2 text-center font-semibold text-indigo-700 bg-indigo-50/30 cursor-pointer hover:bg-indigo-100 transition-colors">
                             <div className="flex items-center justify-center gap-1">
                                 {dateB ? dateB.toLocaleDateString('th-TH', { day: 'numeric', month: 'short' }) : '...'}
                                 <SortIcon col="callRateB" />
                             </div>
                         </th>
                         <th onClick={() => handleSort("diffCall")} className="py-3 px-4 text-center font-bold text-indigo-700 cursor-pointer hover:bg-indigo-200 bg-indigo-100/50 transition-colors">
                             <div className="flex items-center justify-center gap-1">
                     ...
                     <tfoot className="bg-slate-50 border-t-2 border-slate-200 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] sticky bottom-0 z-10">
                         <tr>
                             ...
                             <td className={`px-2 py-4 text-center font-extrabold ${getDiffColor(tableSummary.diffCall)} bg-indigo-100/50 border-r border-white`}>
                                 {tableSummary.diffCall > 0 ? "▲" : tableSummary.diffCall < 0 ? "▼" : ""} {Math.abs(tableSummary.diffCall).toFixed(2)}%
                             </td>
                         </tr>
                     </tfoot>
                 )}
